...
try:
    ...
    try:
        from scraperconfig import PROXY_URL
    except ImportError:
        PROXY_URL = ""
        
    try:
        from scraperconfig import PROXY_USERNAME, PROXY_PASSWORD
    except ImportError:
        PROXY_USERNAME = ""
        PROXY_PASSWORD = ""
    ...
    if ROTATE_PROXIES and PROXY_LIST:
        ...
        if '@' in proxy_url:
            ...
            if len(auth_parts) > 1:
                ...
                if len(auth) > 1:
                    username = auth[0]
                    masked_proxy = f"{protocol}://{username}:****@{parts[1]}"
        
        print(f"🔄 Using rotating proxy: {masked_proxy}")
        ...
        proxy_url = PROXY_URL
        print(f"🌐 Using fixed proxy: {proxy_url.replace(PROXY_PASSWORD, '****') if PROXY_PASSWORD else proxy_url}")
        ...
        print("⚠️ Proxy usage is enabled but no proxy URLs are configured")
        return None
